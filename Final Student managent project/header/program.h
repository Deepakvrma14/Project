...
void edit_program_f ()
{
    ...
    {
        system("CLS");  
        title(); edit_program_title();

        printf("\t\t\t\t\tProgram Code: ");   
        scanf("%s%*c",&edit_program[i].code);
        printf("\n\t\t\t\t\tProgram Name: ");      
        scanf("%[^\n]",&edit_program[i].name);
        ...
        if(permission == 'Y' || permission == 'y')
        {
            ...
            printf("\t\t\t\t\tDo You Edit Another Program ? Y / N : ");
             permission = getch();
